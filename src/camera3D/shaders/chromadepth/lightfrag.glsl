...
void main() {
  ...
  // `color` is the value gl_FragColor would otherwise be
  vec4 color = gl_FrontFacing ? vertColor : backVertColor;

  // I want the hue to come from `rgb` but dim it based on the luminence of `color`
  float luminence = dot(color, vec4(0.2126, 0.7152, 0.0722, 0.0));
